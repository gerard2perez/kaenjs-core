...
export interface CodeTemplateMapping {
	template?:string
	_4xx? : string
	_5xx? : string
	_7xx? : string
...
export  function ErrorToHTML (options:CodeTemplateMapping) {
	return async function ErrorToHTML (ctx:KaenContext) {
		if(ctx.status>=400 && ctx.accepts('text/html')) {
			let key = `_${Math.floor(ctx.status/100)}xx`;
			let template = options[ctx.status] || options[key] || options.template;
			if(template) {
				// @ts-ignore
				await ctx.render(template, codes.find(c=>c.code === ctx.status));
			}
		}
	}
}
